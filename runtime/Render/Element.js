...
(function (ElmRuntime) {
    (function (Render) {
        (function (Element) {
            ...
            var Utils = ElmRuntime.Render.Utils;
            var newElement = Utils.newElement, extract = Utils.extract, addTransform = Utils.addTransform, removeTransform = Utils.removeTransform, fromList = Utils.fromList, eq = Utils.eq;
            ...
            function setProps(props, e) {
                ...
                if (props.hover.ctor !== '_Tuple0') {
                    ...
                    e.addEventListener('mouseout', function (evt) {
                        if (e.contains(evt.toElement || evt.relatedTarget))
                            return;
                        overCount = 0;
                        props.hover(false);
                    });
            ...
            function flow(dir, elist) {
                ...
                switch (dir.ctor) {
                    case "DDown":
                        array.reverse();
                    case "DUp":
                        return flowWith(goDown, array);
                    case "DRight":
                        array.reverse();
                    case "DLeft":
                        return flowWith(goRight, array);
                    case "DOut":
                        array.reverse();
                    case "DIn":
                        return flowWith(goIn, array);
                }
            }
            ...
            function render(elem) {
                return setProps(elem.props, makeElement(elem));
            }
            ...
            function makeElement(e) {
                ...
                switch (elem.ctor) {
                    ...
                    case 'Flow':
                        return flow(elem._0, elem._1);
                    case 'Container':
                        return container(elem._0, elem._1);
                    case 'Spacer':
                        return newElement('div');
                    case 'RawHtml':
                        return rawHtml(elem._0);
                    case 'Custom':
            ...
            function update(node, curr, next) {
                ...
                switch (nextE.ctor) {
                    case "Spacer":
                        break;
                    case "RawHtml":
                        if (nextE._0 !== currE._0)
                            node.innerHTML = nextE._0;
                        break;
                    case "Image":
                        if (nextE._0.ctor === 'Plain') {
                            if (nextE._3 !== currE._3)
                                node.src = nextE._3;
                        } else if (!eq(nextE, currE) || next.props.width !== curr.props.width || next.props.height !== curr.props.height) {
                            node.parentNode.replaceChild(render(next), node);
                            return true;
                        }
                        break;
            ...
            function updateProps(node, curr, next) {
                ...
                if (props.width !== currP.width)
                    e.style.width = (props.width | 0) + 'px';
                if (props.height !== currP.height)
                    e.style.height = (props.height | 0) + 'px';
